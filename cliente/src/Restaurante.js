    ...
    <div style={{ fontFamily: "sans-serif", maxWidth: "500px", margin: "0 auto", padding: "16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px", marginBottom: "20px" }}>
        <div style={{ background: "#FAEEDA", borderRadius: "12px", padding: "12px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "22px", fontWeight: 500, color: "#854F0B", margin: 0 }}>{pedidos.filter(p => p.estado === "pendiente").length}</p>
        </div>
        ...
        <div style={{ background: "#EAF3DE", borderRadius: "12px", padding: "12px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "22px", fontWeight: 500, color: "#3B6D11", margin: 0 }}>{pedidos.filter(p => p.estado === "listo").length}</p>
        </div>
      </div>
      ...
      {pedidos.map((p) => (
        <div key={p.id} style={{ border: "0.5px solid #ddd", borderRadius: "12px", padding: "14px", marginBottom: "12px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {p.estado === "pendiente" && (
              <button onClick={() => cambiarEstado(p.id, "preparando")} style={{ background: "#185FA5", color: "#fff", border: "none", borderRadius: "8px", padding: "10px 16px", cursor: "pointer", fontSize: "13px", fontWeight: 500 }}>
                Aceptar y preparar
              </button>
            )}
            {p.estado === "preparando" && (
            ...
            {p.estado === "asignado" && (
              <div style={{ background: "#F1EFE8", borderRadius: "8px", padding: "10px 16px" }}>
                <p style={{ fontSize: "13px", color: "#5F5E5A", margin: 0 }}>Domiciliario en camino al restaurante</p>
              </div>
            )}
          </div>
        </div>
